 ...
 */
public class F14 extends Problem<DoubleDecisionVariable> {
    @Override
    public F14 init(int dimension) {
    ...
    public void evaluate(Individual inputedIndividual) {//传过来的是DoubleIndividual类型，即决策变量是浮点型
        double fitness = 0, innserSum = 0;
        for (int i = 0; i < dimension; i++) {
            innserSum += inputedIndividual.getDecisionVariable().getGeneCodes()[i];
            fitness += innserSum * innserSum;
        }
    ...
    public String getName() {
        return  FactoryProblems.getName(14, dimension);
    }
