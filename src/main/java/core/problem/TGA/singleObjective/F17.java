 ...
 */
public class F17   extends Problem<DoubleDecisionVariable> {

    @Override
    public F17 init(int dimension) {
    ...
    public void evaluate(Individual inputedIndividual) {//传过来的是DoubleIndividual类型，即决策变量是浮点型
        double fitness=0;
        fitness=1000000*inputedIndividual.getDecisionVariable().getGeneCodes()[0]*inputedIndividual.getDecisionVariable().getGeneCodes()[0];
        for (int i = 1; i < dimension; i++) {
           fitness+=inputedIndividual.getDecisionVariable().getGeneCodes()[i]*inputedIndividual.getDecisionVariable().getGeneCodes()[i];
        }
    ...
    public String getName() {
        return  FactoryProblems.getName(17, dimension);
    }
