 ...
 */
public class F19 extends Problem<DoubleDecisionVariable> {

    @Override
    public F19 init(int dimension) {
    ...
    public void evaluate(Individual inputedIndividual) {//传过来的是DoubleIndividual类型，即决策变量是浮点型
        double fitness = 0, tem = 0;
        for (int i = 1; i < dimension; i++) {
            tem = inputedIndividual.getDecisionVariable().getGeneCodes()[i] + 0.5;
            fitness += tem * tem;
        }
    ...
    public String getName() {
       return  FactoryProblems.getName(19, dimension);
    }
