 ...
 */
public class F24  extends Problem<DoubleDecisionVariable> {

    @Override
    public F24 init(int dimension) {
        super.init(dimension);this.dimension = dimension;
        stopFitness = new double[]{78.3323};
    ...
    public void evaluate(Individual inputedIndividual) {//传过来的是DoubleIndividual类型，即决策变量是浮点型
        double fitness = 0;
        for (int i = 0; i < dimension ; i++) {
            fitness+=Math.pow(inputedIndividual.getDecisionVariable().getGeneCodes()[i],4)
                    -16*Math.pow(inputedIndividual.getDecisionVariable().getGeneCodes()[i],2)
                    +5*inputedIndividual.getDecisionVariable().getGeneCodes()[i];
        }
        inputedIndividual.getDecisionVariable().setFitness(new double[]{-fitness/this.dimension});//把求最小值问题转换为求最大值问题了
    }
    ...
    public String getName() {
        return FactoryProblems.getName(24, dimension);
    }
