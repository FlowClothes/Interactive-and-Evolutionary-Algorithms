 ...
 */
public class F33 extends Problem<DoubleDecisionVariable> {

    @Override
    public F33 init(int dimension) {
    ...
    public void evaluate(Individual inputedIndividual) {//传过来的是DoubleIndividual类型，即决策变量是浮点型
        ...
        for (int i = 0; i < dimension; i++) {
            fitness += Math.abs(
                    inputedIndividual.getDecisionVariable().getGeneCodes()[i] * Math.sin(inputedIndividual.getDecisionVariable().getGeneCodes()[i]) + 0.1 * inputedIndividual.getDecisionVariable().getGeneCodes()[i]);
        }
    ...
    public String getName() {
        return FactoryProblems.getName(33, dimension);
    }
